---
title: "12th Iberian Congress on Prions"
logo: figures/logo24_3.png
---

```{=html}
<div class="row">
  <a href="visitsantiago.qmd" class="location-button bobina1">
    <img src="figures/pin-logo.png" alt="pin" width="20" height="20">
    Santiago de Compostela, Spain
  </a>
  <a href="programme.qmd" class="cal-text bobina2">
    <img src="figures/cal-logo.png" alt="pin" width="20" height="20">
    <strong>23-24 May 2024</strong>
  </a>
  <div class="social-links-sidebar" >
  <a href="https://www.instagram.com/iberianprionconference" class="ig-button bobina3a" target="_blank">
    <img src="figures/ig-icon.png" alt="instagram">
  </a>
  <a href="https://twitter.com/IberianPrion" class="x-button bobina3b" target="_blank">
    <img src="figures/x-logo.png" alt="Twitter">
  </a>
  <a href="https://linkedin.com/company/iberianprionconference" class="ln-button bobina3c" target="_blank">
    <img src="figures/In-logo.png" alt="LinkedIn">
  </a>
  </div>
</div>

<div class='row due'>
  <a href='registration.qmd' class='offerta-speciale bobina4'><strong>Registration open till the 19th of May</strong></a>
  <a href='programme.qmd' class='movete bobina5' style='color: #8ebecd;'><strong>Check the programme!</strong></a>
</div>

<style>
  .descr {
    display: flex;
    align-items: center;
  }
  .row {
    display: flex;
    flex-direction: row;
    gap: 15px;
    align-items: center;
    margin-top: 10px;
  } 
  .location-button {
    text-decoration : none;
    border: 2px solid #dde9f3;
    padding : 4px;
    border-radius: 5px;
  }
  .cal-text {
    display: flex;
    align-items: center;
    background-color: #dde9f3;
    border-radius: 5px;
    padding : 6px;
    color : currentcolor;
    text-decoration : none;
  }

  .hover-additional-text {
    display: none;
    margin-left: 10px; 
    opacity: 0;
    text-decoration: none;
    color : black;
  }
  .social-links-sidebar a:hover img {
    transform: scale(1.2);
    gap: 20px;
  }

  .social-links-sidebar {
    display: flex;
    justify-content: center;
    align-items: center; 
    gap: 15px; 
  }
  .social-links-sidebar a {
    display: flex;
    flex-wrap: wrap;
    gap: 10px;
  }
  .social-links-sidebar img {
    width: 30px;
    height: 30px;
  }

  @media (max-width: 600px) {
    .row {
      flex-direction: column;
      align-items: center;
    }
    .logo-social-container {
      flex-direction: column;
    }
    .social-links-sidebar {
    display: flex;
    flex-wrap: wrap;
    justify-content: center;
    align-items: center;
    align-content: flex-start;
    }
  }
  /* */
  .offerta-speciale {
    border: #0000ff;
    align-items: center;
    background-color: #dde9f3;
    border-radius: 5px;
    padding: 6px;
    /*font-size: 24px;*/
    color : #474747;
    text-decoration : none;
  }
  .movete {
    text-decoration : none;
    border: 2px solid #dde9f3;
    padding : 4px;
    border-radius: 5px;
  }
  .fermate {
    text-decoration : none;
    border: 2px solid #df382c;
    padding : 4px;
    border-radius: 5px;
  }
  .due {
    gap: 10px;
    flex-direction: column;
    align-items: flex-start;
  }
  @media (max-width: 600px) {
    .row {
      flex-direction: column;
      align-items: center;
    }
    .due {
      gap: 15px;
    }
    .logo-social-container {
      flex-direction: column;
    }
    .social-links-sidebar {
    display: flex;
    flex-wrap: wrap;
    justify-content: center;
    align-items: center;
    align-content: flex-start;
    }
  }
  @keyframes zoomInOut {
    0% {
      transform: scale(1);
    }
    50% {
      transform: scale(1.1);
    }
    100% {
      transform: scale(1);
    }
  }
  @media (pointer: coarse) {
    .bobina1 {
    animation: zoomInOut 0.5s;
    animation-delay: 0s;
    }
    .bobina2 {
    animation: zoomInOut 0.5s;
    animation-delay: 0.3s;
    }
    .bobina3a {
    animation: zoomInOut 0.5s;
    animation-delay: 0.6s;
    }
    .bobina3b {
    animation: zoomInOut 0.5s;
    animation-delay: 0.7s;
    }
    .bobina3c {
    animation: zoomInOut 0.5s;
    animation-delay: 0.8s;
    }
    .bobina4 {
    animation: zoomInOut 0.5s;
    animation-delay: 0.9s;
    }
    .bobina5 {
    animation: zoomInOut 0.5s;
    animation-delay: 1.2s;
    }
  }

</style>
```

### Sponsors 

```{=html}
<!-- EACH ROW A SPONSOR!-->
<div class="sponsors">
  <a href="https://www.usc.gal/en" target="_blank"><img src="figures/usc.svg"  /></a>
  <a href="https://fundacionprionicas.org/" target="_blank"><img src="figures/FEEP.jpg"  /></a>
  <a href="https://www.proquinorte.com/" target="_blank"><img src="figures/Logo-Proquinorte.avif"  /></a>
  <a href="https://veterinaryresearch.biomedcentral.com/" target="_blank"><img src="figures/vjl.jpg" /></a>
  <a href="https://www.celtaingenieros.com/" target="_blank"><img src="figures/c.jpg"  /></a>
  <a href="https://www.renfe.com/es/en" class="schermostretto" target="_blank"><img src="figures/renfe.png" style="height: 70px;" /></a>
</div>
<style>
  .treno_dei_desideri {
    display : flex;
    flex-direction : row;
    align-items : center;
    gap : 20px;
    margin-top : 15px;
  }
  .controllo_biglietti {
    display : block;
  }
  .sponsors a:hover img {
    transform: scale(1.05);
    gap: 20px;
  }
  .logotreno {
    display : flex;
  }
  .logotreno img {
    height : 40px;
  }
  .logotreno a:hover img {
    transform: scale(1.1);
    gap: 20px;
  }
  .sponsors {
    display: flex;
    justify-content: center; 
    align-items: center; 
    flex-wrap : wrap; 
    gap: 15px; 
  }
  /*
  .sponsors a {
    display: flex;
    flex-wrap: wrap; 
    gap: 10px; 
  }*/
  .sponsors img {
    height: 100px;
  }
  .schermostretto {
    display : none;
  }
  .schermostretto a { 
    display : none;
  }
  .schermolungo {
    display : inline;
  }
  .logotesto {
    height : 35px;
  }
  @media (max-width: 600px) {
    .sponsors {
    display: flex;
    flex-wrap: wrap;
    justify-content: center;
    align-items: center;
    align-content: flex-start;
    }
    .treno_dei_desideri {
    display: block;
    gap : 0px;
    margin-top : 15px;
    }
    .logotreno {
      display : none;
    }
    .schermostretto {
      display : inline;
    }
  }
  .bottone-copia {
    width : 170px;
    border: 0px solid #dde9f3;
    border-radius: 50px;
    background-color: #dde9f3;
    color : #676767;
    align-items: center;
  }
  .bottone-copia svg {
    width: 1em; /* Adjust this value to your needs */
    vertical-align: top;
  }
  .bottone-copia :hover {
    align-items: center;
    background-color: #dde9f3;
    /*font-size: 24px;*/
    color : #000000;
  }
  .og-bottone-copia {
    display: inline;
  }
  .copied-bottone-copia {
    display: none;
    color : #000000;
  }

</style>
```

```{=html}
<script src="https://cdn.jsdelivr.net/npm/lucide@0.16.12/dist/lucide.js"></script>
<h4>Get the <strong >renfe</strong> 5% discount</h4>
<div class="treno_dei_desideri">
  <div class="logotreno">
    <a href="https://www.renfe.com/es/en" target="_blank"><img src="figures/renfe.png"  /></a>
  </div>
  <div class="controllo_biglietti">
    <li>Select in the <a href="https://www.renfe.com/es/en" target="_blank">RENFE webpage</a> the tickets you want.</li>
    <li>Select the <strong>Congress/Events fee</strong> or <strong><em>Descuento de Congresos/Ferias</em></strong> in the drop-down menu.</li>
    <li>Copy the code 
    <button class="bottone-copia" id="copyButton" alt='copy the discount code!'>
      <span class="og-bottone-copia">
        <strong>TES/2024/00636</strong>
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-copy"><rect width="14" height="14" x="8" y="8" rx="2" ry="2"/><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2"/></svg>
      </span>
      <span class="copied-bottone-copia"><strong>Copied!</strong></span>
    </button>
    in the Authorization Number/ <em>Número de autorización</em> box.</li>
  </div>
</div>

<script>
document.getElementById('copyButton').addEventListener('click', function() {
  var self = this;
  var originalText = this.querySelector('.og-bottone-copia').innerText;
  navigator.clipboard.writeText(originalText).then(function() {
    console.log('Copying to clipboard was successful!');
    self.querySelector('.og-bottone-copia').style.display = 'none';
    self.querySelector('.copied-bottone-copia').style.display = 'inline-block';
    setTimeout(function() {
      self.querySelector('.og-bottone-copia').style.display = 'inline-block';
      self.querySelector('.copied-bottone-copia').style.display = 'none';
    }, 1000); // Change back after 1 second
  }, function(err) {
    console.error('Could not copy text: ', err);
  });
});
</script>
```
### Welcome

Welcome to the 12th Iberian Prion Congress at the Universidad de Santiago - Galicia!

It is with great pleasure that we gather here today, over 20 years since the first case of BSE was diagnosed in Spain, marking a significant turning point in our professional careers. The impact of that event shaped our determination to unravel the mysteries of prion diseases, which continue to pose immense challenges to patients and researchers alike. While a cure remains elusive, the past two decades have witnessed remarkable advancements in the field of prion science.

The Iberian Prion Congress has always been guided by the principles of the One Health approach, bringing together experts from diverse disciplines to address the shared molecular mechanisms of prion diseases and other protein misfolding disorders prevalent in our aging society. This interdisciplinary collaboration has played a crucial role in our quest to develop effective treatments and interventions.

We are proud to host the 12th Iberian Prion Congress at the prestigious Universidad de Santiago - Galicia, where renowned scientists, medical doctors, biologists, veterinarians, biotechnologists, biochemists, and other specialists from the Iberian Peninsula and beyond will convene. This gathering provides a unique platform for fostering and strengthening interdisciplinary collaborations, enabling us to collectively tackle the challenges that lie ahead.

In particular, we are committed to nurturing the participation of young scientists. Their innovative ideas, fresh perspectives, and groundbreaking research are the keys to unlocking new avenues in prion science. The congress will feature dedicated sessions and interactive discussions that offer young researchers the opportunity to present and engage with their peers, fostering an environment of mentorship and knowledge exchange.

As we embark on this scientific journey at the Universidad de Santiago - Galicia, we are not only inspired by the vibrant atmosphere of this historic city but also by the spirit of collaboration and shared purpose that permeates the Iberian Prion Congress. Together, we will strive to transcend boundaries, explore new frontiers, and ultimately find solutions to the challenges posed by prion diseases.

On behalf of the organizing committee, we extend our heartfelt gratitude to all the participants, speakers, sponsors, and volunteers who have contributed to making this congress possible. Your dedication and support are instrumental in our collective pursuit of advancing prion research.

We eagerly await your arrival at the 12th Iberian Prion Congress at the Universidad de Santiago - Galicia. Let us join forces, exchange knowledge, and forge new partnerships that will pave the way for a future where prion diseases are conquered, bringing hope and relief to countless individuals affected by these devastating conditions.

Thank you, and we look forward to welcoming you all in Santiago!

The Organizing Committee


### Important dates

Abstract submission deadline: [**7th April 2024**](abstract.qmd).

Registration deadline: [**19th **May** 2024**](registration.qmd).

Congress: 23-24.05.2024.

<img src="figures/santiago1.jpg" width="200" height="200">

### [Organizing and Scientific Committee](committee.html)

- Leticia Lucero López, CNB-CSIC, Spain
- Giada Lavigna, Istituto Mario Negri, Italy
- Cristina Sampedro, CICbioGUNE, Spain
- Sara Canoyra Sánchez, CISA-INIA-CSIC, Spain
- Leticia Camila Fernández Flores, UMG, Germany
- Anna Burato, SISSA, Italy
- Luigi Celauro, SISSA, Italy
- Michał Burdukiewicz, UMB, Poland/UAB, Spain
- Nuria Lopez Lorenzo, USC, Spain
- Sonia Pérez Lázaro, UNIZAR, Spain
- Cacheiro Vázquez Raúl, USC, Spain
- Sabzehei Sanaz, USC, Spain
- Nicole Innocenti, CIBIO-UNITN, Italy
- Nuno Filipe Coutinho Gonçalves Anjo, CICbioGUNE, Spain
- Dino Gasparotto, UNITN & Sibylla Biotech, Italy
- Ilaria Zeni, CIBIO-UNITN, Italy
- Jesús Requena, USC, Spain


<!-- register now! button cool effect + code on sidebar_mods.css -->
```{=html}
<script>
window.onload = function() {
  var sidebarLinks = document.querySelectorAll('.sidebar-item-container a');
  for (var i = 0; i < sidebarLinks.length; i++) {
    if (sidebarLinks[i].textContent.trim() === 'UPLOAD YOUR ABSTRACT!') {
      sidebarLinks[i].classList.add('abstract-button');
    }
    if (sidebarLinks[i].textContent.trim() === 'REGISTER NOW!') {
      sidebarLinks[i].classList.add('strobelight');
    }
  }
};

</script>
```